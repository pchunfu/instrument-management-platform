...
layui.define(["layer"], function(f) {
	...
	n.showLoading = function(s, r, p, q) {
		...
		var i = ['<div class="ball-loader ' + q + '"><span></span><span></span><span></span><span></span></div>',
			'<div class="rubik-loader ' + q + '"></div>', '<div class="signal-loader ' + q +
			'"><span></span><span></span><span></span><span></span></div>', '<div class="layui-loader ' + q +
	...
	n.openCellAutoExpand = function() {
		...
		i.off("mouseleave.openCellAutoExpand").on("mouseleave.openCellAutoExpand",
	...
	n.events = {
		...
		fullScreen: function() {
			...
			var v = h(this).find("i");
			var s = document.fullscreenElement || document.msFullscreenElement || document.mozFullScreenElement || document.webkitFullscreenElement ||
				false;
			if (s) {
	...
	n.chooseLocation = function(t) {
					...
					if (I === "complete") {
						...
						for (var K = 0; K < L.length; K++) {
							...
							if (J.location !== undefined) {
								M += '<div data-lng="' + J.location.lng + '" data-lat="' + J.location.lat +
						...
						if (K.tips) {
							...
							for (var M = 0; M < J.length; M++) {
								...
								if (O.location !== undefined) {
									N += '<div data-lng="' + O.location.lng + '" data-lat="' + O.location.lat +
										'" class="ew-map-select-search-list-item">';
									N +=
		...
		var q = ['<div class="ew-map-select-tool" style="position: relative;">',
			...
			'     <div id="ew-map-select-tips" class="ew-map-select-search-list layui-hide">', "     </div>", "</div>",
			...
			"     </div>", '     <div id="ew-map-select-pois" class="layui-col-sm5 ew-map-select-search-list">',
			"     </div>", "</div>"
		].join("");
		n.open({
			id: "ew-map-select",
	...
	n.cropImg = function(r) {
		...
		layui.use(["Cropper", "upload"], function() {
			...
			var B = ['<div class="layui-row">', '     <div class="layui-col-sm8" style="min-height: 9rem;">',
				'          <img id="ew-crop-img" src="', x || "", '" style="max-width:100%;" alt=""/>', "     </div>",
				...
				"     </div>", "</div>", '<div class="text-center ew-crop-tool" style="padding: 15px 10px 5px 0;">',
				...
				"     </div>", '     <div class="layui-btn-group" style="margin-bottom: 10px;">',
			...
			var p = {
				...
				parse: function(u, v) {
					...
					try {
						var x = p.exp("^" + i + "#"),
							y = p.exp(s + "$");
						u = u.replace(p.exp(i + "#"), i + "# ").replace(p.exp(s + "}"), "} " + s).replace(/\\/g, "\\\\").replace(p.exp(
							i + "!(.+?)!" + s), function(z) {
							z = z.replace(p.exp("^" + i + "!"), "").replace(p.exp("!" + s), "").replace(p.exp(i + "|" + s), function(
								A) {
	...
	k.tips = function(r, i, q) {
		...
		if (h(i).length > 0 && h(i).parents(".layui-form").length > 0) {
			if (h(i).is("input") || h(i).is("textarea")) {
				...
				if (h(i).hasClass("layui-form-select") || h(i).hasClass("layui-form-radio") || h(i).hasClass(
						"layui-form-checkbox") || h(i).hasClass("layui-form-switch")) {
					p = h(i).prev()
				}
			}
		}
