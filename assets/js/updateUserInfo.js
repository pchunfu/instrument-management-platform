        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.code == 0) {
                    ...
                    if (datas.length > 0) {
                        ...
                        for (var i = 0; i < datas.length; i++) {
                            ...
                            if (item.departmentId != value) {
                                ...
                                $("#department").append("<option value=" + item.departmentId + " selected='selected' >" + item.departmentName + "</option>");
                                key = i;
                            }
                        }
                    } else {
                    ...
                    var postModels;
                    postModels = datas[key].postModels;

                    if (postModels.length > 0) {
                        var value = $("#post option:selected").val();
                        $("#post").empty(); //清空下拉框
                        ...
                        for (var i = 0; i < postModels.length; i++) {
                            ...
                            if (item.postId != value) {
                                ...
                                $("#post").append("<option value=" + item.postId + " selected='selected' >" + item.postName + "</option>");
                            }
                        }
                    } else {
                        $("#post").empty(); //清空下拉框
